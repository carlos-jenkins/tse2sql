...
{% if cantons -%}
INSERT INTO `tsesql`.`canton` (`id_canton`, `name`, `province_id_province`) VALUES
...
-- -----------------------------------------------------
-- Data for table `tsesql`.`voter`
-- -----------------------------------------------------
START TRANSACTION;
USE `tsesql`;

{% if voters -%}
INSERT INTO `tsesql`.`voter` (`id_voter`, `sex`, `id_expiration`, `site`, `name`, `family_name_1`, `family_name_2`, `district_id_district`) VALUES
{%- for voter in voters %}
    ({{voter.id}}, {{voter.sex}}, {{voter.expiration.strftime('%Y-%m-%d')}}, {{voter.site}}, '{{voter.name}}', '{{voter.family_name_1}}', '{{voter.family_name_2}}', {{voter.district}}){% if not loop.last %},{% endif %}
{%- endfor -%}
;
