...
var voterByNameQuery = 'SELECT * FROM voter WHERE MATCH(name, family_name_1, family_name_2) ' +
...
voter.getVoterByName = function(voterName, type, callback) {
    voterName = _.map(voterName.split(' '), function(token) {
        /*
            ...
            indexing engine ignores words shorter than this minimum size, then in
            our case when the length of the token is minor than 3 the tokenOperator
            is not added, otherwise we add the + operator
        */
        var tokenOperator = (token.length < dbMinTokenSize) ? '' : '+';
        return tokenOperator + token;
    }).join(' ');

    db.query(voterByNameQuery, ['\'' + voterName + '\''], function(err, rows, fields) {
        callback(null, rows);
    });
};
